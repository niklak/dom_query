...
fn bench_add_class(c: &mut Criterion) {
    ...
    let test_cases = vec![
        ...
        (
            "medium",
            "p-4 m-4 text-left text-left bg-gray-100 hover:bg-gray-200 
            rounded rounded shadow",
        ),
        (
            "large",
            "p-2 m-2 text-center bg-white hover:bg-gray-100 rounded 
    ...
    for (name, classes) in &test_cases {
        group.bench_with_input(BenchmarkId::new("remove_class", name), &doc, |b, doc| {
            b.iter(|| {
                let d = doc.clone();
                let sel = d.select("td");
                sel.remove_class(classes);
                black_box(sel);
            })
        });
    }
