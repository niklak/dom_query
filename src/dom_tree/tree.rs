...
impl Tree {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        let root_node = nodes.first().ok_or_else(|| "Root node (NodeId(0)) missing".to_string())?;
        ...
        for node in nodes.iter() {
            ...
            if let Some(first_child_id) = node.first_child {
                let first_child = nodes.get(first_child_id.value)
                    .ok_or_else(|| format!("Invalid first_child reference at node {:?}", id))?;
                ...
                if first_child.prev_sibling.is_some() {
                    return Err(format!("first_child {:?} of node {:?} has non-None prev_sibling", first_child_id, id));
                }
                ...
                while let Some(cid) = current_id {
                    let current = nodes.get(cid.value)
                        .ok_or_else(|| format!("Invalid child reference at node {:?}", cid))?;

                    if current.parent != Some(id) {
                        return Err(format!("Child {:?} has incorrect parent, expected {:?}", cid, id));
                    }

                    if let Some(prev) = current.prev_sibling {
                        let prev_node = nodes.get(prev.value)
                            .ok_or_else(|| format!("Invalid prev_sibling reference at node {:?}", prev))?;
                        if prev_node.next_sibling != Some(cid) {
                            return Err(format!("prev_sibling {:?} next_sibling mismatch with {:?}", prev, cid));
                        }
                    }

                    if let Some(next) = current.next_sibling {
                        let next_node = nodes.get(next.value)
                            .ok_or_else(|| format!("Invalid next_sibling reference at node {:?}", next))?;
                        if next_node.prev_sibling != Some(cid) {
                            return Err(format!("next_sibling {:?} prev_sibling mismatch with {:?}", next, cid));
                        }
                ...
                if node.last_child != last_seen {
                    return Err(format!("last_child mismatch at node {:?}, expected {:?}, found {:?}", id, node.last_child, last_seen));
                }
            }

            // Validate last_child linkage
            if let Some(last_child_id) = node.last_child {
                let last_child = nodes.get(last_child_id.value)
                ...
                if last_child.next_sibling.is_some() {
                    return Err(format!("last_child {:?} of node {:?} has non-None next_sibling", last_child_id, id));
                }
            }

            // Validate sibling links
            if let Some(prev_sibling_id) = node.prev_sibling {
                let prev_sibling = nodes.get(prev_sibling_id.value)
                    .ok_or_else(|| format!("Invalid prev_sibling reference at node {:?}", id))?;

                if prev_sibling.next_sibling != Some(id) {
                    return Err(format!("prev_sibling {:?} does not link back to {:?}", prev_sibling_id, id));
                }
            }

            if let Some(next_sibling_id) = node.next_sibling {
                let next_sibling = nodes.get(next_sibling_id.value)
                    .ok_or_else(|| format!("Invalid next_sibling reference at node {:?}", id))?;

                if next_sibling.prev_sibling != Some(id) {
                    return Err(format!("next_sibling {:?} does not link back to {:?}", next_sibling_id, id));
                }
            }
        }
