...
use crate::matcher::{MatchScope, Matcher, Matches, MatchesAlt};
...
impl Document {
    ...
    pub fn select_matcher(&self, matcher: &Matcher) -> Selection {
        ...
        let nodes = MatchesAlt::new(root, matcher).collect();
    ...
    pub fn select_single_matcher(&self, matcher: &Matcher) -> Selection {
        let node = MatchesAlt::new(self.tree.root(), matcher).next();
